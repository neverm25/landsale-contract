...
contract LandBaseToken is ERC721BaseToken {
    ...
    /**
     * @notice Mint a new quad (aligned to a quad tree with size 3, 6 or 12 only)
    ...
    function mintQuad(
        ...
        for (uint256 i = 0; i < size * size; i++) {
            uint256 id_ = _idInPath(i, size, x, y);
            require(_owners[id_] == 0, "Already minted");
            emit Transfer(address(0), to, id_);
        }
    ...
    /**
     * @notice transfer multiple quad (aligned to a quad tree with size 3, 6 or 12 only)
     * @param from current owner of the quad
     * @param to destination
    ...
    function _regroup12x12(
        ...
        if (!ownerOfAll && set) {
            require(_owners[quadId] == uint256(uint160(from)), "not owner of all sub quads nor parent quads");
        }
        _owners[quadId] = uint256(uint160(to));
        return true;
    }
        ...
        if (owner1x1 != 0) {
            ...
            if (owner3x3 != address(0)) {
                ...
                if (owner6x6 != address(0)) {
                    ...
                    return address(uint160(_owners[LAYER_12x12 + (x / 12) * 12 + ((y / 12) * 12) * GRID_SIZE]));
                }
            }
        }
    }
